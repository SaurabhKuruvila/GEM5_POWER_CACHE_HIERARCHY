...
{
  MD5_CTX mdContext;
  unsigned char (*data)[BLOCK_SIZE] = malloc(sizeof(unsigned char*[NUM][BLOCK_SIZE]));
  unsigned int i;
  ...
  for(int iter = 0; iter < NUM; iter++)
  {
    ...
    {
      data[iter][i] = (unsigned char)(i & iter & 0xFF);
    }
  }
  ...
  for(int iter = 0; iter < NUM; iter++)
  {
    ...
    MD5Update (&mdContext, data[iter], BLOCK_SIZE);

    MD5Final (&mdContext);

    printf ("[%d] Digest: ",iter);
    MDPrint (&mdContext);
    printf ("\n");
  }
}
...
static void MDStringTest ()
{
  MDString ("Saurabh Shubham");
  MDString ("Computer Architecture");
